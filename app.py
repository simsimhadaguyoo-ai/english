...
# 🛠️ [버그 수정 2] Upstage Document Parse 정식 엔드포인트 적용
def extract_text_from_image(image_bytes, api_key):
    try:
        ...
        if response.status_code == 200:
            ...
            # Upstage Document Parse 결과 구조에 맞춰 텍스트 추출
            return result.get("content", {}).get("text", "")
        else:
            st.error(f"OCR 인식에 실패했습니다. (서버 응답 코드: {response.status_code}, 내용: {response.text})")
            return None
    except Exception as e:
...
def generate_translation_and_vocab(english_text, api_key, major_info, user_level):
    try:
        ...
        if response.status_code == 200:
            ...
            st.error(f"Solar LLM 서버 통신에 실패했습니다. (응답 코드: {response.status_code})")
            return None
    except Exception as e:
