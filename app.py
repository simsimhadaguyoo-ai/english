...
st.set_page_config(
    page_title="AI 학년별 맞춤 원서 번역기 (B&W)", page_icon="📚", layout="centered"
)

# 🎨 [디자인 고정] 미니멀 매트 블랙 테마 및 표준 CSS 반영
st.markdown(
    """
    <style>
...
# 🛠️ Upstage Document Parse API 연동 함수 (URL 수정 완료)
def extract_text_from_image(image_bytes, api_key):
  try:
    ...
    if response.status_code == 200:
      # 응답 결과에서 텍스트 추출 (Document Parse 구조에 맞게 조정)
      result = response.json()
      # text 필드가 없을 경우 content 등 하위 구조 탐색 안전장치
      return result.get("text", "")
    else:
      st.error(
          f"OCR 인식에 실패했습니다. (서버 응답 코드: {response.status_code}, 메시지:"
          f" {response.text})"
      )
      return None
  except Exception as e:
...
# 🛠️ Upstage Solar LLM API 연동 함수 (URL 수정 완료)
def generate_translation_and_vocab(english_text, api_key, major_info, user_level):
  try:
    ...
    if user_level == "고등학교 1~2학년 (기초/내신)":
      level_instruction = (
          "- 번역 수준: 수능 및 내신 기초를 다지는 고등학생 눈높이입니다. 구조가 복잡한"
          " 문장은 끊어 읽기 형태로 직관적으로 번역해 주고, 문장 구조가 한눈에 보이게 다듬어"
          " 주세요.\n- 단어장 수준: 핵심 '수능 필수 영단어 및 숙어'를 5개 이상"
          " 뽑아주시고, 단어의 뜻과 예문을 포함해 주세요."
      )
      ...
      level_instruction = (
          ...
          " 위주로 발췌하고, 문맥적 해석을 돕는 '구문 독해 팁'을 추가해 주세요."
      )
    elif user_level == "대학 신입생 / 입문자 (학부 1~2학년)":
      level_instruction = (
          "- 번역 수준: 전공 원서를 처음 읽는 저학년 수준입니다. 생소한 학술 전문"
          " 용어가 등장하면 단어 옆에 괄호()를 치고 '일상적인 비유나 쉬운 예시 설명'을"
          " 본문 속에 직접 주입해 주세요.\n- 단어장 수준: 전공 기초 어휘 위주로 뽑고,"
          " 기초 개념 지식을 친절하게 풀어서 기술해 주세요."
      )
    elif user_level == "학부 고학년 (학부 3~4학년)":
      level_instruction = (
          "- 번역 수준: 전공 서적 교재나 학술 논문 번역본 표준 규격에 맞추어, 실제"
          " 세미나에서 사용하는 매끄럽고 세련된 학술적 어조로 의역해 주세요.\n- 단어장"
          " 수준: 학술 전문 용어 위주로 발췌하고, 졸업 연구에 인용 서술할 수 있는"
          " 정확한 전공 학술적 정의를 기재해 주세요."
      )
    elif user_level == "대학원생 / 연구원 (석박사 단계)":
      level_instruction = (
          "- 번역 수준: 실무 및 연구 전문가용입니다. 기초적인 개념 설명은 배제하고,"
          " 해외 유명 저널의 초록(Abstract)처럼 극도로 정교하고 조밀하며 압축된"
          " 프로페셔널한 번역 톤을 유지해 주세요.\n- 단어장 수준: 최신 연구 트렌드와"
          " 직결된 고난도 핵심 키워드 위주로 간결하게 주석을 달아주세요."
      )

    system_prompt = (
        f"너는 학생들이 영어 지문, 전공 서적, 논문을 마스터하도록 돕는 대한민국 최고의 AI"
        f" 맞춤형 교육 멘토야. 현재 문서의 학문 분야는 **[{major_info}]**이며, 독자의"
        f" 학업 레벨은 **[{user_level}]** 단계야.\n\n아래 제시된 학년별 가이드라인을"
        f" 100% 준수하여 두 가지 결과물을 완벽하게 작성해줘:\n\n{level_instruction}\n\n1."
        " [자연스러운 수준별 의역]\n위 지침에 맞춰 문체와 부가설명 깊이를 정밀 조절한"
        " 한국어 해석본을 작성해줘.\n\n2. [필수 개인 맞춤형 단어장]\n본문에서 독자의 현재"
        " 학습 단계에 가장 치명적이고 중요한 어휘를 5개 이상 엄선하여 [단어 - 뜻 -"
        " 맞춤형 가이드 해설] 형태로 요약 단어장을 만들어줘."
    )
    ...
    if response.status_code == 200:
      ...
      st.error(
          f"Solar LLM 서버 통신에 실패했습니다. (코드: {response.status_code})"
      )
      return None
  except Exception as e:
    ...
    if image_bytes is not None:
      ...
      if st.button("🚀 개인 맞춤형 AI 독해 분석 시작", type="primary"):
        ...
        if extracted_text:
          ...
          with st.spinner(
              f"Processing Solar AI (학년별 최적화 가이드 수립 중)..."
          ):
